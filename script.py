...
def extract_step_definitions(ts_code):
    # Regex to find step definitions (Given, When, Then)
    # Captures:
    # 1. Step keyword (Given, When, Then)
    # 2. Step pattern (regex, double-quoted string, or single-quoted string)
    # 3. Parameters of the async function
    # 4. Function body (non-greedy match for content within the outermost braces)
    step_def_regex = re.compile(
        r'(Given|When|Then)\s*\(\s*(?P<pattern>/(?:\.|[^/])+/[a-zA-Z]*|"[^"]*"|\'[^\']*\')\s*,\s*(async\s*)?\(([^)]*)\)\s*=>\s*{(?P<body>.*?)}\s*\)\s*;',
        re.DOTALL  # Allows '.' to match newline characters
    )
    
    definitions = []
    for match in step_def_regex.finditer(ts_code):
        pattern = match.group('pattern')
        body = match.group('body')
        # The full match gives a good representation of the step definition's signature for reporting
        full_signature = match.group(0)
        # Extract just the signature part for concise reporting if needed, up to the body
        signature_for_report = full_signature.split("=> {")[0]
        definitions.append({'pattern': pattern, 'body': body, 'signature_for_report': signature_for_report})
    return definitions
...
typescript_content = """/* eslint-disable max-len */
...
step_definitions = extract_step_definitions(typescript_content)
normalized_bodies = defaultdict(list)

for definition in step_definitions:
    # Using the more descriptive signature for reporting duplicates
    signature_snippet = definition['signature_for_report']
    
    normalized = normalize_body(definition['body'])
    if normalized: 
        normalized_bodies[normalized].append(signature_snippet)

duplicate_groups = {k: v for k, v in normalized_bodies.items() if len(v) > 1}

report = "Duplicate Step Definition Bodies Report:\n\n"

if not duplicate_groups:
    report += "No step definitions found with identical function bodies after normalization.\n"
else:
    group_num = 1
    for body, signatures in duplicate_groups.items():
        report += f"Group {group_num} of Step Definitions with Identical Bodies:\n"
        report += "Step Definition Signatures:\n"
        for sig_snippet in signatures:
            report += f"  - {sig_snippet.strip()}\n" # strip to remove any leading/trailing whitespace from the snippet
        report += "Shared Normalized Body:\n"
        report += "-" * 30 + "\n"
        # Replace escaped newlines in the body for printing, if the body itself is multi-line
        report += body.replace('\\n', '\n') + "\n"
        report += "-" * 30 + "\n\n"
        group_num += 1
